...
- **Revit 2026** (.NET 8) — full feature set, all tools
- **Revit 2024** (.NET Framework 4.8) — same plugin UI and tool surface as Revit 2026, **except IFC Space-to-Room** (held back for now — see below)
...
Both builds share the same plugin window, ribbon button, and approval workflow — a write tool queues in the **Pending** tab and waits for Approve/Reject exactly like on 2026, and destructive tools (`revit_delete_views`, `revit_delete_sheets`) always require manual approval regardless of Direct Edit mode, on both versions.
