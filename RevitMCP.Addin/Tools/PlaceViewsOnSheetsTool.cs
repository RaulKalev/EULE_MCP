...
public class PlaceViewsOnSheetsTool : IRevitMcpTool
{
    ...
    public string Description =>
        ...
        "Optional: sheetIds (long array) or allSheets (bool=true), " +
    ...
    public Task<McpToolResult> ExecuteAsync(UIApplication uiapp, McpToolRequest request, CancellationToken cancellationToken)
    {
        ...
        var toPlace = proposals.Where(p => p.SheetId.HasValue).ToList();
        if (toPlace.Count == 0)
            return Task.FromResult(Fail(request, "No views could be matched to sheets. Run preview first."));
        ...
        foreach (var p in toPlace)
        {
            try
            {
                ...
                if (sheet == null || view == null) { warnings.Add($"Could not resolve view/sheet for view {p.ViewId}"); continue; }

                var center = PlacementPointResolver.GetSheetCenter(doc, sheet);
                Viewport.Create(doc, sheet.Id, view.Id, center);
                placed++;
                placed_.Add(new { viewId = p.ViewId, viewName = p.ViewName, sheetId = p.SheetId, sheetNumber = p.SheetNumber });
            }
            catch (Exception ex)
            {
                warnings.Add($"Failed to place '{p.ViewName}': {ex.Message}");
            }
        }
        t.Commit();
        ...
        return Task.FromResult(new McpToolResult
        {
            ...
            Success    = placed > 0,
            Message    = $"Placed {placed}/{toPlace.Count} view(s) on sheets.",
